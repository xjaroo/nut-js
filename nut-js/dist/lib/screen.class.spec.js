...
const loggingMock = (0, sneer_1.mockPartial)({
    debug: jest.fn(),
    info: jest.fn(),
    warn: jest.fn(),
    error: jest.fn(),
    ...
    describe("find", () => {
        describe("queries", () => {
            ...
            it("should choose the correct finder implementation for color queries", async () => {
                // GIVEN
                const matchPoint = new point_class_1.Point(0, 0);
                const matchResult = new match_result_class_1.MatchResult(0.99, matchPoint);
                const SUT = new screen_class_1.ScreenClass(providerRegistryMock);
                const needle = {
                    id: "color-query",
                    type: "color",
                    by: {
                        color: new rgba_class_1.RGBA(255, 0, 255, 1),
                    },
                };
                const findMatchMock = jest.fn(() => Promise.resolve(matchResult));
                providerRegistryMock.getColorFinder = jest.fn(() => (0, sneer_1.mockPartial)({
                    findMatch: findMatchMock,
                }));
                providerRegistryMock.getLogProvider = () => new noop_log_provider_class_1.NoopLogProvider();
                // WHEN
        ...
        it("should throw on invalid search input", async () => {
            ...
            // THEN
            await expect(result).rejects.toThrowError(/find requires an Image, a text query, a color query or a window query.*/);
        });
    ...
    describe("findAll", () => {
        describe("queries", () => {
            it("should throw on invalid search input", async () => {
                ...
                // THEN
                await expect(result).rejects.toThrowError(/findAll requires an Image, a text query, a color query or a window query.*/);
            });
            it("should choose the correct finder implementation for images", async () => {
                // GIVEN
                const matchResult = new match_result_class_1.MatchResult(0.99, searchRegion);
