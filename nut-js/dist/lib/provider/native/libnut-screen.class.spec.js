    ...
    describe("screen data", () => {
        ...
        it("should resolve when screenshot data is available", async () => {
            ...
            libnut.screen.capture = jest.fn(() => ({
                ...
                image: Buffer.from(new Array(screenShotSize.width * screenShotSize.height * 4 + 10).fill(0)),
            ...
            expect(image.data.length).toEqual(screenShotSize.width * screenShotSize.height * 4);
            expect(libnut.screen.capture).toBeCalledTimes(1);
        });
